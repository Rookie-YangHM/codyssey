...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------- |
| **Django**  | - 풀 스택 [프레임워크](#프레임워크) ([batteries-included](#batteries-included))<br>- [ORM](#ORM), 관리자 페이지, 인증 시스템 등 내장<br>- ["Don't Repeat Yourself" (DRY) 원칙](#DRY_원칙) 강조            | - 빠른 개발 속도<br>- 강력한 보안 기능<br>- 대규모 프로젝트에 적합<br>- 활발한 커뮤니티와 풍부한 문서                                                            | - [학습 곡선](#학습곡선)이 비교적 높음<br>- 소규모 프로젝트에는 다소 무거울 수 있음      | - 복잡하고 규모가 큰 웹 애플리케이션<br>- CMS, CRM, 소셜 네트워크 등        |
| **Flask**   | - [마이크로 프레임워크](#마이크로_프레임워크)<br>- 최소한의 기능만 제공<br>- 확장을 통해 기능 추가 (플러그인)                                                                              | - 가볍고 유연함<br>- 배우기 쉽고 시작하기 빠름<br>- 소규모 및 중간 규모 프로젝트에 적합<br>- [API 서버](#API서버) 구축에 용이                                              | - 필요한 기능을 직접 구성해야 함<br>- 대규모 프로젝트에서는 관리가 복잡해질 수 있음    | - 소규모 웹 애플리케이션<br>- [RESTful API 서버](#RESTful_API_서버)<br>- [프로토타입 개발](#프로토타입_개발) |
| **FastAPI** | - 최신 고성능 웹 프레임워크<br>- [Starlette(ASGI)](#Starlette(ASGI)) 기반, [Pydantic](#Pydantic)으로 데이터 유효성 검사 및 [직렬화](#직렬화)<br>- [비동기](#비동기) ([async](#async)/[await](#await)) 지원 | - 매우 빠른 성능 (Node.js, Go에 필적)<br>- 자동 API 문서 생성 ([Swagger UI](#Swagger_UI), [ReDoc](#ReDoc))<br>- [타입 힌트 기반](#타입_힌트_기반)으로 코드 자동 완성 및 유효성 검사 | - 비교적 새로운 프레임워크로 커뮤니티 규모가 작음<br>- 비동기 프로그래밍에 대한 이해 필요 | - 고성능 API 서버<br>- 마이크로서비스<br>- 데이터 과학 및 머신러닝 API      |
...
> <a name="프레임워크"></a>**프레임워크(Framework)** - 컴퓨터 프로그램을 만들 때 쓰는 **'미리 준비된 블록 세트'**, **'미리 준비된 도구 상자'**<br><br>
...
> <a name="batteries-included"></a>**batteries-included**
...
> <a name="DRY_원칙"></a>**DRY 원칙 (Don't Repeat Yourself)**
...
> <a name="학습곡선"></a>**학습 곡선 (Learning Curve)**
...
> <a name="마이크로_프레임워크"></a>**마이크로 프레임워크 (Microframework)**
...
> <a name="API서버"></a>**API 서버 (API Server)**
...
> <a name="RESTful_API_서버"></a>**RESTful API 서버 (RESTful API Server)**
...
> <a name="프로토타입_개발"></a>**프로토타입 개발 (Prototype Development)**
> 프로토타입 개발은 본격적인 제품을 만들기 전에, 핵심 기능만 빠르게 구현해서 미리 써볼 수 있는 시제품을 만드는 과정이에요. 마치 실제 건물을 짓기 전에 작은 모형을 만들어보거나, 새 옷을 만들기 전에 가봉을 해보는 것과 같아요.



> <a name="Starlette(ASGI)"></a>**Starlette(ASGI)**
...
> <a name="직렬화"></a>**직렬화 (Serialization)**
...
> <a name="async"></a>**async (비동기 함수 선언 키워드)**
...
> <a name="await"></a>**await (비동기 작업 대기 키워드)**
...
> <a name="Swagger_UI"></a>**Swagger UI**
...
> <a name="타입_힌트_기반"></a>**타입 힌트 기반 (Type Hinting Based)**
